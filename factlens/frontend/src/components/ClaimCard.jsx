...
function EvidenceMetric({ label, value }) {
  return (
    <div className="min-w-0 rounded-2xl border border-white/6 bg-white/4 px-3 py-2.5 sm:px-4 sm:py-3">
      <p className="text-[10px] font-semibold uppercase tracking-[0.24em] text-slate-500 sm:text-[11px]">{label}</p>
...
function EvidenceSection({ title, subtitle, items, tone }) {
  ...
  return (
    <section className={`rounded-[1.35rem] border p-3 sm:p-4 ${tone} animate-fade-in-up`}>
      <div className="mb-3 flex flex-wrap items-start justify-between gap-3 sm:mb-4">
        ...
        <span className="glass-pill shrink-0 rounded-full px-3 py-1 text-xs font-medium text-white/80">
      ...
      <div className="space-y-3">
        {items.map((source) => (
          <article
            ...
            className="glass-card rounded-[1.2rem] p-3 sm:p-4"
          >
            <div className="flex flex-col gap-3">
              ...
              <div className="grid w-full grid-cols-3 gap-2 rounded-2xl border border-white/6 bg-white/3 p-2 sm:p-3">
...
function ClaimCard({ result, claim, anchorId }) {
  ...
  return (
    <article
      id={anchorId}
      className={`glass-card-static rounded-[1.75rem] p-4 animate-fade-in-up sm:p-5 ${verdictStyle.glow}`}
    >
      <div className="flex flex-col gap-4">
        <div className="min-w-0">
          ...
          <blockquote className="mt-4 border-l-2 border-blue-400/30 pl-4 text-sm leading-7 text-slate-200 sm:mt-5 sm:text-base">
            {result.claim}
          </blockquote>
      ...
      <div className="mt-5 grid grid-cols-2 gap-2 sm:grid-cols-3 xl:grid-cols-6 sm:gap-3">
      ...
      {queryVariants.length ? (
        <section className="mt-5 glass-card rounded-[1.35rem] p-3 sm:p-4">
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {queryVariants.map((query) => (
              <div
                ...
                className="min-w-0 rounded-2xl border border-white/6 bg-white/3 px-3 py-2"
              >
      ...
      {riskFlags.length ? (
        <section className={`mt-5 rounded-[1.35rem] border p-3 sm:p-4 ${verdictStyle.panel}`}>
          <div className="flex items-center gap-2">
