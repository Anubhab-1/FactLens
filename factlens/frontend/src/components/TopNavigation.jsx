import { BookOpenText, History, Search, Zap } from "lucide-react";
    ...
    <header className="glass-card-static sticky top-4 z-20 rounded-[1.75rem] px-4 py-3 sm:px-5 animate-fade-in-up gradient-border">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex items-center gap-3">
          <Link
            to="/"
            className="group flex h-11 w-11 items-center justify-center rounded-2xl bg-gradient-to-br from-blue-500/20 to-purple-500/20 text-blue-300 ring-1 ring-inset ring-blue-400/20 transition-all duration-300 hover:scale-110 hover:shadow-lg hover:shadow-blue-500/20"
          >
            <Search className="h-5 w-5 transition-transform duration-300 group-hover:rotate-12" />
          </Link>

          <div>
            <Link to="/" className="font-display text-3xl leading-none text-white hover:text-gradient transition-all duration-300">
              FactLens
            </Link>
